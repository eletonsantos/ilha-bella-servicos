...
export default function InvoiceUploadForm({ closingId, competence, totalValue }: Props) {
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <button
          ...
          className="w-full bg-brand-blue hover:bg-brand-blue-dark text-white font-semibold
                     py-3 rounded-xl transition-all disabled:opacity-50 flex items-center justify-center gap-2"
        >
          {loading ? 'Enviando...' : (<><Upload size={16} /> Enviar nota fiscal</>)}
        </button>
      </form>
    </div>
  )

...
export default function DeleteFechamentoButton({ closingId, competence, techName }: Props) {
  ...
  async function handleDelete() {
    const confirmed = confirm(
      ...
      `Esta ação é IRREVERSÍVEL e removerá o fechamento, ` +
      `a nota fiscal enviada, contestação e solicitação de antecipação vinculadas.\n\n` +
    ...
    const reconfirmed = confirm(
      `Confirme novamente: excluir permanentemente o fechamento "${competence}" de ${techName}?`
    )
    ...
    setError('')
    try {
      const res = await fetch(`/api/admin/fechamentos/${closingId}`, { method: 'DELETE' })
      if (!res.ok) {
  ...
  return (
    <div className="card p-6 border-l-4 border-red-400">
      ...
      <p className="text-sm text-slate-500 mb-4">
        ...
        e solicitação de antecipação associadas. Não é possível desfazer.
      </p>
      ...
      >
        ...
        {loading ? 'Excluindo...' : 'Excluir fechamento'}
      </button>
    </div>
  )
}

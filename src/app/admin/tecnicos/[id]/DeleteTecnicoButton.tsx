...
export default function DeleteTecnicoButton({ techId, techName }: Props) {
  ...
  async function handleDelete() {
    const confirmed = confirm(
      `Tem certeza que deseja EXCLUIR o cadastro de "${techName}"?\n\nEsta ação é irreversível e removerá todos os fechamentos e notas fiscais associados.`
    )
    if (!confirmed) return
    ...
    setError('')
    try {
      const res = await fetch(`/api/admin/tecnicos/${techId}`, { method: 'DELETE' })
      if (!res.ok) {
  ...
  return (
    <div className="card p-6 border-l-4 border-red-400">
      ...
      >
        ...
        {loading ? 'Excluindo...' : 'Excluir técnico'}
      </button>
    </div>
  )
}
